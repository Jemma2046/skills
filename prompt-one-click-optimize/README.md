...
只需在对话中表达优化提示词的意图即可，无需手动加载。典型触发语：
...
不改变核心意图 — 优化是强化，不是替换。用户想说什么，必须说得更到位，而不是换成另一件事。
可直接使用 — 优化后的提示词拿来就能用，不需要二次加工。
通用优先 — 如用户未指定模型，按通用最佳实践优化；指定了模型，则针对性适配。
避免过度工程化 — 提示词不是越长越好，冗余的结构反而稀释核心指令。每一句话都要有存在的理由。
